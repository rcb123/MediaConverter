...
'undefined' == typeof window
	? (self.addEventListener('install', () => self.skipWaiting()),
		...
		self.addEventListener('fetch', function (e) {
			...
			const s =
				coepCredentialless && 'no-cors' === o.mode ? new Request(o, { credentials: 'omit' }) : o;
			e.respondWith(
				fetch(s)
					.then((e) => {
						...
						return (
							...
							coepCredentialless || o.set('Cross-Origin-Resource-Policy', 'cross-origin'),
							o.set('Cross-Origin-Opener-Policy', 'same-origin'),
							new Response(e.body, { status: e.status, statusText: e.statusText, headers: o })
						);
					})
	...
	: (() => {
			...
			if (t) {
				...
				r.serviceWorker.controller.postMessage({
					...
					s.shouldDeregister() && r.serviceWorker.controller.postMessage({ type: 'deregister' });
			}
				...
				(window.isSecureContext
					? r.serviceWorker
						? r.serviceWorker.register(window.document.currentScript.src).then(
								(e) => {
									!s.quiet && console.log('COOP/COEP Service Worker registered', e.scope),
								...
								(e) => {
									!s.quiet && console.error('COOP/COEP Service Worker failed to register:', e);
								}
							)
						: !s.quiet &&
